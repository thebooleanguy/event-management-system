<template>
    <div class="flex items-center justify-center min-h-screen bg-[#E5E5E5]">
        <form @submit.prevent="handleSubmit" class="w-full max-w-lg bg-[#FFFFFF] p-10 rounded-3xl shadow-xl space-y-8">
            <h2 class="text-3xl font-bold text-[#1A1A1A] text-center">Register Your Account</h2>
            <p v-if="error" class="text-red-500 text-center">{{ error }}</p>
            <div class="space-y-6">
                <div class="relative">
                    <label for="name" class="block text-sm font-medium text-[#1A1A1A]">Name</label>
                    <div
                        class="flex items-center border border-[#D1D5DB] rounded-md shadow-sm focus-within:ring-2 focus-within:ring-[#7DD3FC]">
                        <font-awesome-icon :icon="faUser" class="text-[#1E40AF] ml-3" />
            ...
            <p class="text-sm text-center text-[#1A1A1A] mt-6">
                Already have an account? <router-link to="/users/login" class="text-[#7DD3FC] hover:underline">Login
                    here</router-link>
            </p>
        </form>
    </div>
...
<style scoped>
/* Add any specific styles needed for your form */
input {
    max-width: 100%;
